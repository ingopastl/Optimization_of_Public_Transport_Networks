...
public class Main {
            ...
            while (true) {
                try {
                    for (i = previousI; i < 40; i++) {
                        ...
                        /*
                         * Estamos colocando o MaxIterations do algoritmo como 10 porque queremos salvar o progresso em
                         * memória persistente a cada 10 iterações do algoritmo. Ao total faremos 40 execuções do
                         * algoritmo com 10 iterações máximas, o que totaliza em 400 gerações.
                         */

                        initialPopulation = loadInitialPopulation(problem);
                        algorithm = new NSGAIIIBuilder<>(problem)
                                .setPopulationSize(92)
                                .setMaxIterations(10)
    ...
    private static void printFinalSolutionSet(List<? extends Solution<?>> population, int totalProgress) {
        boolean wereDirsCreated =  new File("results" + File.separatorChar + totalProgress).mkdirs();
        ...
        new SolutionListOutput(population)
                ...
                .setFunFileOutputContext(new DefaultFileOutputContext("results" + File.separatorChar + totalProgress + File.separatorChar + "FUN.tsv"))
                .print();
    ...
    private static List<DoubleSolution> loadInitialPopulation(PTDJMetalProblem problem) throws IOException {
        ...
        File f = new File("lastPopulation.txt");
